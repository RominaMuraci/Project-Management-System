    ...
    <div id="content-wrapper">
        ...
        <div class="container-fluid">
            ...
            <div class="table-responsive">
                <table id="archivedProjects" class="table table-striped table-bordered ">
                    <thead>
                    <tr>
...
<script>
    $(document).ready(function () {
        ...
        const table = $('#archivedProjects').DataTable({
            processing: true,
            serverSide: false,
            paging: true,
            pageLength: 10,
            ajax: {
                url: 'fetch_archived_projects.php',
        ...
        `;
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        if (isAdmin) {
                            return `
                            <button class="button-blue-rect edit-btn" data-id="${row.id}">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="button-red-rect delete-btn" data-id="${row.id}">
                                <i class="fas fa-trash-alt"></i>
                            </button>
                        `;
                        } else {
                            return '';  // Hide actions for non-admins
                        }
                    },
                    targets: -1, // Ensure this applies to the last column (Actions)
                    visible: isAdmin // Hide the entire column if the user is not an admin
                }
            ],
            order: [[1, 'desc']]
        });

        // This ensures the header is hidden for non-admins
        if (!isAdmin) {
            $('.actions-column').hide();
        }
...
// Apply filter if projectName exists
        if (projectName) {
            setTimeout(function () {
                $('#archivedProjects_filter input').val(projectName).trigger('keyup'); // Apply search filter
    ...
    `;
        ...
        $('#archivedProjects thead input').on('keyup change', function () {
